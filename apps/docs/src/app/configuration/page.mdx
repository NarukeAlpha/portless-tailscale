...
`bun dev` at the root runs turbo, which runs `portless` in each package. Portless detects the package manager and runs `bun run dev:app` through the proxy. No `turbo.json` changes are needed.

People without portless installed can run `bun run dev:app` directly.
